...
#pander(paste0(">> (x) 交易數據: ",Xfile,", 共 ",head(X),"\n\n"));  
pander(paste0(">> (1) 交易數據: ",Xfile,", 共 ",dim(X)[1],"筆紀錄\n\n"));    #84008 row (not include header)
pander(">> (2) 交易數據樣貌為: "); 
kable(head(X)) %>%
...
pander(paste0(">> (1) 時間區間: ",paste(range(X$date),collapse="--"),"\n\n"));   #datetime的範圍(range)
```
...
pander(paste0(">> (1) 客戶人數: ",length(unique(X$cID)),"\n\n") );   
pander(paste0(">> (2) 客戶造訪頻率: ",paste(range(table(X$cID)),collapse="--"),"\n\n")) 
pander(paste0(">> (3) 交易紀錄數大於200次的客戶有: \n\n")) 

kable(table(X$cID)[table(X$cID)>200]) %>%
  kable_styling(c("striped", "bordered")) %>% kable_styling(full_width = F) %>% column_spec(1, bold = T)
```
...
```{r, echo=FALSE, warning=FALSE}
pander(paste0(">> (1) 商品 product 項目: ",length(unique(X$product)),"筆\n\n") );  
pander(paste0(">> (2) 商品 category 類別: ",length(unique(X$category)),"筆\n\n") ); 
pander(paste0(">> (3) 商品 category 分布為: \n\n")) 
...
Cv$DD0 = cut( Cv$DD, breaks=c(-Inf,1,9,99,999,9999,Inf));  
Cv$FF0 = cut( Cv$FF, breaks=c(-Inf,1,10,30,50,100,Inf),labels = c("1次","10次","30次","50次","100次","More"));  
# table(Cv$FF0) #FF--造訪頻次
Cv$MM0 = cut( Cv$MM, breaks=c(-Inf,1000,3000,5000,10000,Inf)); # table(Cv$MM0) #MM--消費金額
Cv$BB0 = cut( as.numeric(Cv$BB), breaks=c(-Inf,7,30,100,Inf));  # table(Cv$BB0) #BB--回購週期
