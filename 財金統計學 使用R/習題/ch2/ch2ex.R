...
p = c(45,48,47.5,46)
n = length(p)
ma = numeric(n-1);ma    #numeric?
for(i in 2:n){
 ma[i-1] = (p[i-1]+p[i])/2
}
ma

largan = read.table("/data/largan.txt")
...
windows()
par(mfrow=c(1,2))
plot(p,type="l",xlab="Time",ylab="?馬?L??",main="?j?ߥ??馬?L???P30?駡?u",lwd=2)
zeros = rep(0,n-1) # rep???ܭ???,?Y0????n-1??
map1 = c(zeros,map)
lines(1:T,map1,lty=2,col="red",lwd=2)
legend("topleft","30?駡?u",col=c("red"),lty=2,bty="n",lwd=2)
plot(p,type="l",xlab="Time",ylab="?馬?L??",xlim=c((T-99),T),lwd=2,
    main="?j?ߥ??馬?L???P30?????u (?̪?100?ӥ?????)")
lines(1:T,map1,lty=2,col="red",lwd=2)
legend("bottomleft","30?駡?u",col=c("red"),lty=2,bty="n",lwd=2)
...
windows()
hist(rt1,breaks=100,prob=T,xlab="",main=expression(paste("???p??",bar(p),"?��t")),lwd=2)
...
windows()
plot(mas2_lrt,xlab="Time",ylab="?????ܲ???",main="?j?ߥ????Ƥ????S?v???????ܲ??? (n = 30)",lwd=2)
...
# 1/4/2000-7/10/2014
# ?x?n?q (TSMC) ?馬?L?? (???v???վ???)
tsmcpper = read.table("c:\\meiyih\\finstats\\ch1\\tsmcpper.txt") # Ū??????
tsmcp = tsmcpper[,1] # ?O?Ĥ@?欰?ѻ?
...
windows()
hist(volall,breaks=100,xlab="?i?ʲv",main="TSMC?i?ʲv?????Ƥ��t (250??)",lwd=2)
summary(volall)
...
windows()
hist(volall,breaks=100,xlab="?i?ʲv",main="LARGAN?i?ʲv?????Ƥ��t (250??)",lwd=2)
summary(volall)
hisvol = hist(volall,plot=FALSE)
hisvol
...
# ex5
# 1/4/2005-4/28/2015
zrt = (lrt-mean(lrt))/sd(lrt) # ?зǤ?
windows()
plot(density(zrt),xlab="?зǤƫᤧ?????Ƴ??S?v",main="?j?ߥ??????Ƴ??S?v?P?`?A?��t??????",lwd=2)
z = sort(zrt)
lines(z,dnorm(z),lty=2,col="blue",lwd=2)
legend("topleft",c("?j?ߥ??????Ƴ??S?v?????pPDF","?зǱ`?A?��t??PDF"),lty=1:2,lwd=2,
...
largan = read.table("c:\\meiyih\\Finstats\\ex\\ch1\\largan.txt")
p = largan[,1]
plot(density(p),xlab="?j?ߥ??馬?L??",main="?j?ߥ??馬?L???ǦC?????p??PDF",lwd=2)
skewness(p)
kurtosis(p)
...
windows()
plot(density(kur),xlab="?p?A?Y??",main="?j?ߥ??????S?v?????p???p?A?Y?Ƥ��t",lwd=2)

# 2-4
# 2-4-1-1
# ex1
TWIm = read.table("c:\\meiyih\\Finstats\\ch2\\TWIm.txt",header=T)#???ɤ????W??
names(TWIm) # TWIm?????U?ǦC?W??
attach(TWIm) # ???????ɮ?
...
# ex2
library(plotrix)
plot(c(0,10),c(0,10),type="n",xlab="???I",ylab="???S",main="?????H???L?????u")
...
sdp = sqrt(varp)
windows()
plot(sdp,mp,type="l",xlim=c(6,10),ylim=c(0,10),xlab="???I",ylab="???S",lwd=3,
     main="?????i?H???겣?զX?",frame.plot=F,axes=T)
...
# ex4
windows()
plot(sdp,mp,type="l",xlim=c(0,10),ylim=c(0,10),xlab="???I",ylab="???S",
     main="?????i?H???겣?զX?",frame.plot=F,axes=F,lwd=3)
rf = 2/12 # ?L???I?Q?v
...
windows()
plot(sdp,mp,type="l",xlim=c(0,10),ylim=c(0,10),xlab="???I",ylab="???S",
     main="?????i?H???겣?զX?",frame.plot=T,axes=T,lwd=3)
text(7.7,5.6,labels="B",pos=1)
points(7.7,5.6,lwd=2,pch=18)
...
hist(xbar,breaks=100,prob=T,xlab="",main="?W?ظg?٦????v???˥????????ˤ��t, n = 30")
lines(density(xbar), col="red", lty=2)
hist(xsd,breaks=100,prob=T,xlab="",main="?W?ظg?٦????v???˥??ܲ????ˤ��t, n = 30")
...
hist(xbar,breaks=100,prob=T,xlab="",main="?W?ظg?٦????v???˥????????ˤ��t, n = 50")
lines(density(xbar), col="red", lty=2)
hist(xsd,breaks=100,prob=T,xlab="",main="?W?ظg?٦????v???˥??ܲ????ˤ��t, n = 50")
lines(density(xsd), col="red", lty=2)
...
hist(xbar,breaks=100,prob=T,xlab="",main="?q?f???Ȳv???˥????????ˤ��t, n = 30")
lines(density(xbar), col="red", lty=2)
hist(xsd,breaks=100,prob=T,xlab="",main="?q?f???Ȳv???˥??ܲ????ˤ��t, n = 30")
lines(density(xsd), col="red", lty=2)
...
hist(xbar,breaks=100,prob=T,xlab="",main="?q?f???Ȳv???˥????????ˤ��t, n = 50")
lines(density(xbar), col="red", lty=2)
hist(xsd,breaks=100,prob=T,xlab="",main="?q?f???Ȳv???˥??ܲ????ˤ��t, n = 50")
lines(density(xsd), col="red", lty=2)
...
T = length(lr)
library(fBasics)
basicStats(lr) # ?p?A???W?B?p?A

# ex4,5
# 1/2/2002-8/29/2014 ?ײv
...
windows()
par(mfrow=c(3,1))
plot(xr,type="l",xlab="",ylab="daily log returns",main="?s?x?????????ײv???????ܰʲv
  ???ɶ?????")
xr = (xr-mean(xr))/sd(xr)
hist(xr,breaks=50,prob=T,xlab="?зǤƫ????Ƴ??S?v",main="?s?x?????????ײv???????ܰʲv???????ϻP??
  ?Ǫ??A?��t")
sxr = sort(xr)
...
legend("topleft",c("?зǱ`?A"),col="dark gray", lty=1, bty="n",lwd=2)

zr = 100*diff(log(z))
windows()
par(mfrow=c(3,1))
plot(zr,type="l",xlab="",ylab="daily log returns",main="?s?x?????H?��??ײv???????ܰʲv
  ???ɶ?????")
zr = (zr-mean(zr))/sd(zr)
hist(zr,breaks=50,prob=T,xlab="?зǤƫ????Ƴ??S?v",main="?s?x?????H?��??ײv???????ܰʲv???????ϻP??
  ?Ǫ??A?��t")
szr = sort(zr)
...
plot(sdp,mp,type="l",lwd=2, xlim=c(2,8), ylim=c(0,0.8),
     xlab=expression(sigma[p]), ylab=expression(mu[p]),main="?Ĳv?e?t?u (?S??????)")
...
plot(sdp,mp,type="l",lwd=2, xlim=c(2,8),ylim=c(-0.2,1),
     xlab=expression(sigma[p]), ylab=expression(mu[p]),main="?Ĳv?e?t?u (??????)")
points(sdx,mx,col="red", lwd=3)
...
windows()
par(mfrow=c(2,1))
ts.plot(bp, type="l", col="1", main="2013/12?x???��f????",lwd=2)
...
legend("bottomright", c("?}?L","?̰?","?̤p","???L","????","?{?f"),
       lty=1:6, col=c("1","2","3","4","5","6"), bty="n")
ts.plot(basic,ylab="", main="???t",lwd=2)
...
twid = twid[,1]
# 9/1/2014 ?O???????v?i?ʲv???? (?C????) 8?I46-13?I45??
...
length(vol)
windows()
hist(vol,breaks=20,xlab="",main="9/1/2014 ?O???????v?i?ʲv???Ƥ????Ƥ��t
 (?C????:8?I46-13?I45??)", col="steelblue",lwd=2)
...
???g = 100*diff(log(japan))
?W?? = 100*diff(log(china))
?x?W = 100*diff(log(taiwan))
x1 = 100*diff(log(sp500))
x2 = 100*diff(log(japan))
x3 = 100*diff(log(china))
x4 = 100*diff(log(taiwan))
all1 = cbind(SP,???g,?W??,?x?W) # ???X??
all = cbind(x1,x2,x3,x4)
windows()
pairs(all1, col="tomato",lwd=2) # ???蠟???????G??
cor(all1)
...
text(sd(x2),mean(x2),labels="???g",pos=3)
points(sd(x3),mean(x3),pch=20)
text(sd(x3),mean(x3),labels="?W??",pos=1)
points(sd(x4),mean(x4),pch=20)
text(sd(x4),mean(x4),labels="?x?W",pos=1)
